    ...
    try {
      ...
      const solutions = await Promise.all(
        questionIds.map(async (questionId) => {
          try {
            const response = await fetch(`${URL_BASE}assessment/solutions/${questionId}`, {
              method: "GET",
              headers: {
            ...
            if (!response.ok) {
              console.error(`Failed to fetch solution for question ${questionId}:`, {
            ...
            console.error(`Error fetching solution for question ${questionId}:`, error);
            throw error;
          }
        })
      );
